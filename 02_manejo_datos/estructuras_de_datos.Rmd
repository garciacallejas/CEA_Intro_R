...
```{r}
# una lista con dos elementos numéricos, un carácter, y un vector de caracteres
...
```{r}
mi.info <- list(dia = 18, mes = 4, nombre = "David", musica = c("extremoduro", "radio tarifa", "tool"))
mi.info
```
...
mis.viajes <- list("2020" = c("Grazalema", "Parc del Cadí", "Cerdeña"), "2019" = c("Portugal", "Doñana", "Picos de Europa"))
mis.viajes
...
# seguramente el dataframe más usado en todos los tutoriales de R... características de diferentes especies de "Iris"
head(iris)
...
# no se puede añadir elemento a elemento, porque no existe la fila 3 del dataframe
# info.personal$dia[3] <- 5

# lo ideal es crear un nuevo dataframe y añadirlo completo con la función "rbind" (row-wise bind, literalmente enganche de filas). 
# Es como apilar piezas de Lego
nueva.fila <- data.frame(dia = 1, mes = 5, nombre = "Sofia")
info.personal <- rbind(info.personal,nueva.fila)

# La manera óptima de trabajar con dataframes es crear desde cero la estructura, con todas las filas que necesitemos. 
# Esto permite a R asignar la memoria necesaria para esos datos, aunque estén temporalmente "vacíos".

personas <- 1000

info.long <- data.frame(dia = numeric(personas), mes = numeric(personas), nombre = character(personas),stringsAsFactors = FALSE)
...
# fijáos que lo que estamos asignando es un vector de caracteres. R asume que si mezclas números y caracteres en un mismo vector, todo será convertido a carácter.
mi.vector <- c(1,2,"Mario")
str(mi.vector)

# Si asignamos esto a una fila de nuestro dataframe, convertirá las columnas numéricas en columnas de caracteres.
...
# al igual que en listas y dataframes, si tenemos una matriz nombrada, podemos acceder a sus elementos por sus nombres
...
# hemos intentado transformar a matriz un dataframe que contenía números y caracteres. R lo convierte en una matriz de caracteres
head(mi.matriz.2)
```
...
# Crea un dataframe de 1000 filas en el que la primera columna sea una columna numérica que siga una distribución normal con media 0 y desviación típica 1. La segunda columna debe ser una distribución aleatoria de las letras del alfabeto. Pista: usa las funciones "rnorm" y "runif", y el vector "letters"
