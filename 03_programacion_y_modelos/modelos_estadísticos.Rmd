...
```{r}
# para el primer ejemplo, veremos la relación entre la profundidad y la magnitud de nuestros datos de terremotos
eq <- read.csv2("../data/Earthquake_data.csv",header = TRUE,dec = ".",stringsAsFactors = FALSE)
head(eq)

# en la siguiente sesión aprenderemos a visualizar datos. Por ahora, fijaos en el resultado de esto
library(tidyverse)
...
# nuestra primera regresión lineal! ¿viene la magnitud de un terremoto explicada por su profundidad?
...
str(m1)
# mucha información, pero la clave es la primera línea... el resultado de llamar a la función lm es una lista!
...
personajes_SW <- read.csv2(file = "../data/starwars_info_personajes.csv",header = TRUE,stringsAsFactors = FALSE)
head(personajes_SW)
...
# literalmente: cuál es el nombre (personajes_SW$name) de la fila cuyo peso (mass) es igual al peso máximo de cualquiera de las filas (función "max").
...
```{r}
personajes_2 <- personajes_SW[-peso.max,] # eliminamos sólo la fila que corresponde al peso máximo
...
Algunos recursos para aprender mucha más estadística con R:
