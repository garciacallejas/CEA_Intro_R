...
if(mi.columna %in% names(eq)){ # esta es una manera estándar de comprobar si una columna está en un dataframe
  
  # si tenemos esta columna, podemos tener otra condición dentro de este código
  if(sum(is.na(eq[,mi.columna])) == 0){ # esta condición pregunta literalmente "si la suma de NAs en la columna Dep es igual a cero"
    
    resultado <- mean(eq[,mi.columna])
    
  }else{ # si no se cumple la condición es que hay al menos un NA en la columna Dep
    
    num.nas <- sum(is.na(eq[,mi.columna])) # ¿cuántos NA?
    datos.validos <- sum(!is.na(eq[,mi.columna])) # y la condición contraria, ¿cuántos no son NA?
    cat(paste("existen",num.nas,"NAs en los datos, la media se calculará con",datos.validos,"observaciones"))
    
    resultado <- mean(eq[,mi.columna],na.rm = TRUE) # el argumento na.rm elimina los NA antes de calcular la media
...
for(contador in 1:10000){
  # repetimos 10000 veces un cierto código con una variable interna que se llama "contador".
}
```
...
# las funciones nrow() y length() nos devuelven el número de filas y columnas de un dataframe, respectivamente
num.personajes <- nrow(naves_SW)

# este será mi vector de resultados. Un elemento por cada personaje, que almacene el número de vehículos que conduce.
...
for(pers in 1:num.personajes){
  # puedo usar el contador "pers" para trabajar en la fila adecuada del dataframe
  # a la vez, selecciono las columnas que me interesan
  # recordad: dataframe[fila(s),columna(s)]
  mis.naves <- naves_SW[pers,2:11]
  
  # usamos la función rowSums, porque la variable "mis.naves" es también un dataframe
  # fijaos que podemos usar nuestro contador "pers" para decir en qué posición del vector
  # de resultados guardamos los datos.
...
# esta operación se aplica de manera automática a todos los elementos del vector
# esto es vectorización
resultado <- mi_vector^2

# si R no tuviera esta funcionalidad, podríamos crear un bucle para hacer el mismo cálculo
...
# el argumento MARGIN especifica si queremos aplicar una función sobre cada fila (1) o sobre cada columna (2)
...
# Fijáos que no hemos necesitado eliminar los NAs de los datos. La función "mean" tiene un argumento na.rm que permite eliminarlos automáticamente. 
# Este argumento (o cualquier otro que tenga la función que queramos usar) también se puede pasar a lapply, como hemos visto.
# Si no le pasamos ese argumento extra, la función "mean" fallará si encuentra NAs
...
```{r}
# EJERCICIO 1
# Crea una matriz de 100x3 en la que cada columna esté formada por números aleatorios, la primera entre 0-1, la segunda entre 0-10, la tercera entre 0-100.
# pista: ?runif
